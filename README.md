...
Subject (User, Group, or ServiceAccount)	The person holding a card
Role	The list of doors a type of card can open — but only in one building (namespace)
ClusterRole	A card-template that works the same way in every building on the campus, or opens doors that don't belong to any single building (like the parking garage — a cluster-scoped resource)
RoleBinding	The act of actually handing a specific person a specific card, valid for one building
ClusterRoleBinding	Handing a specific person a card valid campus-wide
